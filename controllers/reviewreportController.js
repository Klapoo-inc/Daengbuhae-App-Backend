const ReviewReportDao = require('../daos/reviewreportDao');
const ReviewReportDto = require('../dtos/reviewreportDto');
...
const createReviewReport = async (req, res) => {
    try {
        const request = ReviewReportDto.fromRequest_create(req);
        const RecviewReport = await ReviewReportDao.Create(request);
        res.status(200).json(RecviewReport);
    } catch (error) {
        res.status(500).json({ message: 'Internal Server Error', error });
    }
};
...
module.exports = {createReviewReport};
